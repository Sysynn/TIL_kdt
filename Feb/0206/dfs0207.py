...
for _ in range(M):
    node1, node2 = list(map(int,input().split()))
    # node1의 인접 리스트에 node2를 추가
    graph[node1].append(node2)
    # node2의 인접 리스트에 node1을 추가
...
# 중복을 확인할때 사용하는 자료구조 set 데이터 타입을 활용해서 방문 여부 확인 가능
# set의 탐색 시간 복잡도 -> 0(1)
...
# 템플릿 4번
while len(stack) != 0: # 스택의 길이가 0이 아니면
    # 템플릿 5 - 1
    current_node = stack.pop()
