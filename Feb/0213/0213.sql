...
-- 문제 12
SELECT jobTitle, COUNT(*) AS count_job FROM employees GROUP BY jobTitle ORDER BY count_job DESC, jobTitle DESC;
-- 문제 13
SELECT country, COUNT(*) AS count_country FROM customers GROUP BY country ORDER BY count_country DESC, country DESC;
-- 문제 14
SELECT orderNumber, SUM(quantityOrdered * priceEach) AS total FROM orderdetails GROUP BY orderNumber ORDER BY total DESC;
-- 문제 15
...
-- 문제 17
SELECT ordernumber, SUM(quantityOrdered) AS itemsCount, SUM(priceEach * quantityOrdered) AS total FROM orderdetails GROUP BY orderNumber HAVING itemsCount >= 500 AND total >= 50000;